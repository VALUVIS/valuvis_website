...
const DropdownMenu: React.FC<DropdownMenuProps> = ({ title, links, path }) => (
  <li className="relative flex flex-col items-center justify-center h-full">
    {title === "Mehr" ? (
      <div className='peer text-gray-600 hover:text-gray-900 transition-colors h-full'>
        {title}
      </div>
    ) : (
      <Link href={path} className='peer text-gray-600 hover:text-gray-900 transition-colors h-full'>
        {title}
      </Link>
    )}
